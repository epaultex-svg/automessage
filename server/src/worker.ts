...
function canonicalizeModel(model: string): string {
  ...
  return LEGACY_MODEL_ALIASES.get(requestedModel) ?? requestedModel;
}

function isAllowedModel(model: string): boolean {
  return ALLOWED_MODELS.has(canonicalizeModel(model));
}

function buildUpstreamBody(email: ParsedEmail, settings: Settings) {
  return {
    model: canonicalizeModel(settings.model),
    messages: [
...
function validateBody(body: unknown): body is RequestBody {
  ...
  if (model !== "" && !isAllowedModel(model)) return false;
